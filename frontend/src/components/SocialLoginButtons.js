  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-center gap-3 py-3.5 px-4 rounded-full
               bg-white border border-warm-200 hover:bg-warm-50 transition-all text-sm font-semibold text-warm-700
               disabled:opacity-50"
    >
...
export function AppleLoginButton({ onSuccess, redirectTo = '/' }) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="w-full flex items-center justify-center gap-3 py-3.5 px-4 rounded-full
               bg-black hover:opacity-90 shadow-medium transition-all text-sm font-bold text-white
               disabled:opacity-50"
    >
...
// ── Google Sign-In Button (Compact — icon only) ──

export function GoogleLoginButtonCompact({ onSuccess, redirectTo = '/' }) {
  const router = useRouter();
  ...
  async function handleGoogleLogin() {
    ...
    try {
      const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
      if (!clientId) throw new Error('Google-inloggning är inte konfigurerad.');
      if (typeof window !== 'undefined' && window.google?.accounts?.id) {
  ...
  async function handleCredentialResponse(response) {
    try {
      const { user, isNewUser } = await googleLogin(response.credential);
      onSuccess?.({ user, isNewUser });
      router.push(isNewUser ? '/tutorial' : redirectTo);
...
// ── Apple Sign-In Button (Compact — icon only) ──

export function AppleLoginButtonCompact({ onSuccess, redirectTo = '/' }) {
  const router = useRouter();
  const { appleLogin } = useAuthStore();
  const [loading, setLoading] = useState(false);
  ...
  async function handleAppleLogin() {
    ...
    try {
      if (typeof window === 'undefined' || !window.AppleID?.auth) {
        throw new Error('Apple-inloggning är inte tillgänglig.');
      }
      const clientId = process.env.NEXT_PUBLIC_APPLE_CLIENT_ID;
      if (!clientId) throw new Error('Apple-inloggning är inte konfigurerad.');
      const response = await window.AppleID.auth.signIn({
        clientId, scope: 'name email', redirectURI: window.location.origin, usePopup: true,
      });
      const { user, isNewUser } = await appleLogin(
        response.authorization.id_token, response.authorization.code, response.user
      );
      onSuccess?.({ user, isNewUser });
      router.push(isNewUser ? '/tutorial' : redirectTo);
    } catch (err) {
      if (err.message !== 'popup_closed_by_user') console.error('Apple login error:', err);
  ...
  return (
    <button
      ...
      disabled={loading}
      className="flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-full
               bg-black hover:opacity-90 transition-all text-sm font-bold text-white
               disabled:opacity-50"
    >
...
// ── Compact Social Login (side-by-side for register) ──

export function SocialLoginSectionCompact({ redirectTo }) {
  return (
    <div>
      <div className="flex gap-3">
        <GoogleLoginButtonCompact redirectTo={redirectTo} />
        <AppleLoginButtonCompact redirectTo={redirectTo} />
      </div>
      <div className="flex items-center gap-3 my-3">
        <div className="flex-1 h-px bg-warm-300" />
        <span className="text-xs text-warm-500 font-semibold tracking-wider">ELLER</span>
