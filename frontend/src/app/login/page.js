      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
            className="input-field" placeholder="din@email.se" required autoFocus />
        </div>
        <div>
    ...
    <div className="min-h-[calc(100vh-200px)] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8 animate-fade-up">
          <div className="w-14 h-14 bg-brand-400 rounded-full flex items-center justify-center text-white text-2xl mx-auto mb-4">🍳</div>
        ...
        <p className="text-center text-sm text-warm-500 mt-5">
          Har du inget konto? <Link href="/register" className="text-brand-400 font-semibold hover:underline">Skapa konto gratis</Link>
        </p>
      </div>
    </div>
  );
